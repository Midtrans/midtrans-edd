...
function edd_midtrans_gateway_offinstallment_add_settings($settings) {
	...
	$midtrans_offinstallment_settings = array(
		...
		array(
			...
			'type' => 'text',
			'default' => 'Credit Card Installment for any Bank via Midtrans',
		),
		array(
	...
	if(!$errors) { 
		...
		**********************************/
        ...
        if($params['transaction_details']['gross_amount'] >= $edd_options['mt_offinstallment_min_amount'])
        {
          ...
          if (strlen($edd_options["mt_offinstallment_acquiring_bank"]) > 0){
            $mt_params['credit_card']['bank'] = $edd_options["mt_offinstallment_acquiring_bank"];
          }
