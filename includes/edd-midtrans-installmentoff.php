...
function edd_midtrans_gateway_offinstallment_register($gateways)
{
    global $edd_options;
    ...
    $gateways['midtrans_offinstallment'] = array(
        ...
        'checkout_label' => __($checkout_label_offinstallment, 'edd-midtrans')
    );
    return $gateways;
}

add_filter('edd_payment_gateways', 'edd_midtrans_gateway_offinstallment_register');

/**
 * Add  section for payment option Midtrans Offline Installment Payment
 */
function edd_midtrans_gateway_offinstallment_settings_section($sections)
{
    $sections['midtrans_offinstallment'] = __('Midtrans Offline Installment', 'edd-midtrans');
    return $sections;
}

add_filter('edd_settings_sections_gateways', 'edd_midtrans_gateway_offinstallment_settings_section');

function edd_midtrans_gateway_offinstallment_cc_form($purchase_data)
{
    global $edd_options;
...
add_action('edd_midtrans_offinstallment_cc_form', 'edd_midtrans_gateway_offinstallment_cc_form');


function edd_midtrans_gateway_offinstallment_add_settings($settings)
{
    ...
    $midtrans_offinstallment_settings = array(
        array(
            'id' => 'edd_midtrans_offinstallment_gateway_settings',
            'name' => '<strong>' . __('Midtrans Offline Installment Settings', 'edd-midtrans') . '</strong>',
            'desc' => __('Configure the gateway settings', 'edd-midtrans'),
            'type' => 'header'
        ),
        array(
            'id' => 'mt_checkout_label_offinstallment',
            'name' => __('Checkout Label Offline Installment', 'edd-midtrans'),
            'desc' => __('<br>Payment gateway text label that will be shown as payment options to your customers (Default = "Credit Card Installment for any Bank via Midtrans")', 'edd-midtrans'),
            'type' => 'text',
        ),
        array(
        ...
        array(
            'id' => 'mt_offinstallment_installment_term',
            'name' => __('Installment Terms', 'edd-midtrans'),
            'desc' => __('<br>Input the desired Installment Terms. Separate with coma. e.g: 3,6,12', 'edd-midtrans'),
            'type' => 'text',
            'default' => '3,6,12',
        ),
        array(
            'id' => 'mt_offinstallment_acquiring_bank',
            'name' => __('Acquiring Bank', 'edd-midtrans'),
            'desc' => __('<br>Input the desired acquiring bank. e.g: bni </br><b>Leave blank if you are not sure</b>', 'edd-midtrans'),
            'type' => 'text',
        ),
        array(
        ...
        array(
            ...
            'desc' => __('<br>This will allow you to set custom fields that will be displayed on Midtrans dashboard. <br>Up to 3 fields are available, separate by coma (,) <br> Example: Order from web, Processed', 'midtran_offinstallment', 'edd-midtrans'),
            'type' => 'text',
        ),
    );
...
add_filter('edd_settings_gateways', 'edd_midtrans_gateway_offinstallment_add_settings');

function edd_midtrans_gateway_offinstallment_plugin_action_links($links)
{
...
add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'edd_midtrans_gateway_offinstallment_plugin_action_links');
...
function edd_midtrans_gateway_offinstallment_payment($purchase_data)
{
    ...
    if (!$errors) {
        ...
        // add bin params
        $bin_numbers = isset($edd_options['mt_offinstallment_bin_number']) ? strlen($edd_options['mt_offinstallment_bin_number']) : 0;
        $bins = array();
        if ($bin_numbers > 0) {
            $bins = explode(',', $bin_numbers);
        }
        ...
        $mt_params = array(
            ...
            'credit_card' => array(
                ...
                'whitelist_bins' => !empty($bins) ? $bins : '',
            ),
        ...
        if ($mt_params['transaction_details']['gross_amount'] >= $edd_options['mt_offinstallment_min_amount']) {
            ...
            $offinstallment_acquiring_bank = isset($edd_options['offinstallment_acquiring_bank']) ? $edd_options["mt_offinstallment_acquiring_bank"] : 0;
            if ( strlen($offinstallment_acquiring_bank) > 0) {
                $mt_params['credit_card']['bank'] = $offinstallment_acquiring_bank;
            }

            // Add installment param
            $mt_params['credit_card']['installment']['required'] = true;
            $mt_params['credit_card']['installment']['terms'] =
                array(
                    'offline' => $terms
                );
        }
        ...
        if ($edd_options["mt_offinstallment_enable_redirect"]) {
            ...
            try {
                ...
                <script type="text/javascript">(function (c, a) {
                    ...
                    mixpanel.init("<?php echo $mixpanel_key ?>");</script>
                <!-- end Mixpanel -->

                <script src="<?php echo $snap_script_url; ?>" data-client-key="<?php echo $client_key; ?>"></script>
                <center>
                    <p><b><h2 class="alert alert-info">Please complete your payment...</h2></b></p>
...
add_action('edd_gateway_midtrans_offinstallment', 'edd_midtrans_gateway_offinstallment_payment');
