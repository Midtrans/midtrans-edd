...
function edd_midtrans_gateway_add_settings($settings)
{
    ...
    $midtrans_settings = array(
        ...
        array(
            ...
            'desc' => __('<br>Customize allowed payment method, separate payment method code with coma. e.g: bank_transfer, credit_card,  mandiri_clickpay,  cimb_clicks,  bca_klikbca,  bca_klikpay,  bri_epay,  telkomsel_cash,  echannel,  permata_va,  other_va,  bca_va,  bni_va,  indomaret,  danamon_online,  akulaku<br> <b>Leave it default if you are not sure</b>', 'edd-midtrans'),
            'type' => 'text',
        ),
        array(
            'id' => 'mt_custom_expiry',
...
function edd_midtrans_gateway_payment($purchase_data)
{
    ...
    if (!$errors) {
        ...
        $mt_enabled_payment = isset($edd_options['mt_enabled_payment']) ? $edd_options['mt_enabled_payment'] : '';
        $enable_payment = [];
        if (strlen($mt_enabled_payment) > 0) {
            $enable_payment = explode(',', $mt_enabled_payment);
        }
        ...
        $mt_params = array(
            ...
            'enabled_payments' => !empty($enable_payment) ? $enable_payment : array(),
            'credit_card' => array(
        ...
        $enable_redirect = isset($edd_options["mt_enable_redirect"]) ? $edd_options["mt_enable_redirect"] : false;
        if ($enable_redirect) {
